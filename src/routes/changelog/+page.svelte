<script lang="ts">
  import {config} from "@/lib/config/client";

  const versions = config.changelog?.versions ?? [];
</script>

<svelte:head>
  <title>Changelog | TRESR</title>
</svelte:head>

<div class="min-h-screen pt-20 pb-10">
  <div class="container mx-auto max-w-3xl px-4">
    <h1
      class="text-primary font-display mb-12 text-center text-5xl font-black tracking-widest drop-shadow-[0_0_10px_rgba(var(--primary),0.8)]"
    >
      CHANGELOG
    </h1>

    <!-- Ron Jay Briefing -->
    <div class="mb-12 w-full">
      <div class="chat chat-start w-full">
        <!-- Official DaisyUI chat-image places the avatar exactly where the tail points -->
        <div class="chat-image avatar translate-y-3 items-end sm:translate-y-4">
          <div
            class="mask mask-squircle w-16 shadow-lg drop-shadow-[0_0_5px_rgba(var(--primary),0.3)] sm:w-20"
          >
            <img
              src="/assets/images/ron_jay_speech_bubble.webp"
              alt="Ron Jay"
            />
          </div>
        </div>
        <div class="chat-header mb-1 font-bold opacity-80">
          Ron Jay
          <span class="text-xs font-normal opacity-50">Patch Notes</span>
        </div>
        <div
          class="chat-bubble chat-bubble-primary text-left text-sm whitespace-pre-line sm:text-base"
        >
          Listen up, Degen. Here's what we shipped.
          <br />
          Every update, every fix, every buff and nerf — it's all here.
        </div>
      </div>
    </div>

    <!-- Timeline -->
    <ul class="timeline timeline-vertical timeline-compact">
      {#each versions as release, i}
        <li>
          <!-- Line coming into the node from above -->
          {#if i > 0}
            <hr class={i === 1 ? "bg-primary" : "bg-base-content opacity-20"} />
          {/if}

          <div class="timeline-middle">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 20 20"
              fill="currentColor"
              class={`h-5 w-5 ${i === 0 ? "text-primary" : "opacity-20"}`}
            >
              <path
                fill-rule="evenodd"
                d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z"
                clip-rule="evenodd"
              />
            </svg>
          </div>
          <div class="timeline-end w-full pb-8">
            <div
              class={`card ${i === 0 ? "border-primary/30 bg-base-200/70" : "border-base-content/10 bg-base-200/40"} border`}
            >
              <div class="card-body gap-3 p-4 sm:p-6">
                <!-- Version header -->
                <div class="flex flex-wrap items-center gap-3">
                  <span
                    class={`badge ${i === 0 ? "badge-primary" : "badge-ghost"} badge-lg font-mono font-bold`}
                  >
                    v{release.version}
                  </span>
                  {#if i === 0}
                    <span
                      class="badge badge-secondary badge-soft badge-sm animate-pulse"
                    >
                      LATEST
                    </span>
                  {/if}
                  <span class="font-mono text-xs opacity-50">
                    {release.date}
                  </span>
                </div>

                <!-- Title -->
                <h2
                  class={`text-lg font-bold tracking-wide ${i === 0 ? "text-primary" : "opacity-70"}`}
                >
                  {release.title}
                </h2>

                <!-- Notes -->
                <ul class="space-y-1.5">
                  {#each release.notes as note}
                    <li class="flex items-start gap-2 text-sm">
                      <span
                        class="text-primary mt-0.5 shrink-0 font-mono text-xs"
                      >
                        &#9656;
                      </span>
                      <span class="opacity-80">{note}</span>
                    </li>
                  {/each}
                </ul>
              </div>
            </div>
          </div>

          <!-- Line continuing down from the node -->
          {#if i < versions.length - 1}
            <hr class={i === 0 ? "bg-primary" : "bg-base-content opacity-20"} />
          {/if}
        </li>
      {/each}
    </ul>

    {#if versions.length === 0}
      <div class="text-center font-mono opacity-50">
        No changelog entries yet.
      </div>
    {/if}

    <div class="mt-12 text-center">
      <a href="/" class="btn btn-outline btn-primary font-mono tracking-widest">
        RETURN HOME
      </a>
    </div>
  </div>
</div>

<style>
  /*
    DaisyUI's timeline-compact sometimes fails to stretch the <hr>
    across large content blocks. We force it to grow here.
  */
  :global(.timeline.timeline-vertical hr) {
    min-height: 100%;
    height: 100%;
  }
</style>
